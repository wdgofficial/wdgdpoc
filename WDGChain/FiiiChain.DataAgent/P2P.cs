...
namespace FiiiChain.DataAgent
{
    public class P2P
    {
        ...
        public void Start(Guid guid, string ipText, bool isTracker)
        {
            this.IPText = ipText;
            ...
            IPEndPoint ip = new IPEndPoint(ipAddress, int.Parse(GlobalParameters.IsTestnet ? Resource.TestnetPort : Resource.MainnetPort));

            if (isTracker)
            {
                ip = new IPEndPoint(ipAddress, int.Parse(GlobalParameters.IsTestnet ? Resource.TestnetTrackerPort : Resource.MainnetTrackerPort));
            }
            ...
            //handleReceivedDataTimer.Start();
            Thread receiverThread = new Thread(startHandleReceivedData);
            receiverThread.Start();

            Thread senderThread = new Thread(startSendCommand);
            senderThread.Start();
        }
        public void Stop()
        {
                    ...
                    try
                    {
                        ...
                        if (this.sendCommandQueue.TryDequeue(out item))
                        {
                            if (item != null && item.Command != null)
                            {
                                ...
                                LogHelper.Debug($"Send {item.Command.CommandName} to {item.Address }:{ item.Port }  queue[{sendCommandQueue.Count}]");
                            }
        ...
        private void startHandleReceivedData()
        {
            while (this.server != null && this.IsRunning)
            {
                try
                {
                    ...
                    var peer = this.Peers.FirstOrDefault(p => p.IP == item.IP && p.Port == item.Port);
                    ...
                    if (buffer.Length > 4)
                    {
                        ...
                        else if (P2PPacket.BytesEquals(P2PPacket.DefaultPacketPrefixBytes, prefix))
                        {
                            ...
                            else if (!packetItem.Packets.ContainsKey(packet.Index))
                            {
                                packetItem.Packets.Add(packet.Index, packet);
                                packetItem.Timestamp = Time.EpochTime;
                            }
                    ...
                    if (isBufferEnd)
                    {
                        ...
                        if (command != null)
                        {
                            ...
                            //stopwatch.Stop();
                            //LogHelper.Warn($"命令 {command.CommandName} Handle 耗时(毫秒) :: " + stopwatch.ElapsedMilliseconds);
                        }
                    }
                }
                catch (Exception ex)
        ...
        private void receiveDataAsync(IAsyncResult ar)
        {
            ...
            finally
            {
                if (this.IsRunning && this.server != null)
                {
                    ...
                    {
                        this.Stop();
                        this.Start(Guid.Parse(this.Identity), this.IPText, this.isTrackerNode);
                    }
                }
                else
                {
