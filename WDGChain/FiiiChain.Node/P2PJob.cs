...
using static FiiiChain.Messages.CommandNames;
...
namespace FiiiChain.Node
{
    public class P2PJob : BaseJob
    {
        ...
        public override void Start()
        {
            ...
            thread = new Thread(new ParameterizedThreadStart(this.startP2P));
            //thread.IsBackground = true;
            thread.Start(IPAddress);
            //threadProcessLongTimeCommand = new Thread(processLongTimeCommand);
            //threadProcessLongTimeCommand.IsBackground = true;
            //threadProcessLongTimeCommand.Start();
        ...
        public BlockChainInfo GetLatestBlockChainInfo()
        {
            ...
            if (result.IsP2PRunning)
            {
                ...
                result.ConnectionCount = nodes.Count(n => n.IsConnected);
                result.LastBlockHeightInCurrentNode = GlobalParameters.LocalHeight;
                result.LastBlockTimeInCurrentNode = GlobalParameters.LatestBlockTime;
        ...
        private void startP2P(object ip)
        {
            ...
            p2pComponent.P2PStart(this.Identity, Convert.ToString(ip), false);
        }
        ...
        private void TxSyncTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            foreach (var node in nodes)
            {
                if (node.IsConnected)
                {
                    ...
                    if (this.syncManager.TaskCount == 0 || (this.RemoteLatestHeight > -1 && this.RemoteLatestHeight - GlobalParameters.LocalHeight <= 10))
                    {
        ...
        private void blockSyncTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            if (newHeight > GlobalParameters.LocalHeight)
            {
                GlobalParameters.LocalHeight = newHeight;
                localHeightUpdatedTime = Time.EpochTime;
            }

            LocalConfirmedHeight = this.blockComponent.GetLatestConfirmedHeight();
            this.syncManager.LocalBlockHeight = GlobalParameters.LocalHeight;
            GlobalParameters.LatestBlockTime = localHeightUpdatedTime; //GlobalParameters.LatestBlockTime;

            p2pComponent.SetBlockHeightAndTime(GlobalParameters.LocalHeight, GlobalParameters.LatestBlockTime);
        ...
        {
            if (RemoteLatestHeight > GlobalParameters.LocalHeight)
            {
                ...
                if (syncManager.TaskCount < syncManager.MaxTasks)
                {
                    ...
                    if (startHeight <= GlobalParameters.LocalHeight
                        || (!this.tempBlockList.Any(b => b.Header.Height == GlobalParameters.LocalHeight + 1) &&
                        !this.syncManager.TaskList.Any(t => t.Heights.Contains(GlobalParameters.LocalHeight + 1)))
                        )
                    {
                        startHeight = GlobalParameters.LocalHeight + 1;
                    }
                    ...
                    if (startHeight > GlobalParameters.LocalHeight + 1 && tempBlockList.Count > 50)
                    {
        ...
        private void sendHeartbeat(string address, int port)
        {
            ...
            msg.Height = GlobalParameters.LocalHeight;
            msg.BlockTime = GlobalParameters.LatestBlockTime;
        ...
        {
            if(this.RemoteLatestHeight > 0 && (this.RemoteLatestHeight - GlobalParameters.LocalHeight < 500))
            {
        ...
        private void receivedGetTransaction(string address, int port, GetTxsMsg msg, int nonce)
        {
            ...
            var txs = this.txComponent.GetTransactionMsgsFromPool(msg.Hashes);
            if (txs != null)
            {
                txList.AddRange(txs);
            }
        ...
        {
            if (this.txComponent.CheckBlackTxExisted(msg.Hash))//黑名单存在
                return;
            if (this.newTxInDownloading.Contains(msg.Hash))//正在下载中
                return;
            if (this.txComponent.CheckTxExisted(msg.Hash))//存在于数据库
                return;
        ...
        {
            (new MiningPoolComponent()).UpdateMiningPools(msg.MinerInfos);
        }
        private void receivedNewMiningPoolMessage(P2PState state, NewMiningPoolMsg msg)
        {
            if (msg == null)
                return;
            if (state == null || (new MiningPoolComponent()).AddMiningToPool(msg.MinerInfo))
            {
                var nodes = this.p2pComponent.GetNodes();
                nodes.ForEach(peer =>
                {
        ...
        private void receivedGetHeight(string address, int port, int nonce)
        {
            ...
            var block = this.blockComponent.GetBlockEntiytByHeight(height);

            if (block != null)
            {
                var payload = new HeightMsg();
                payload.Height = height;
                payload.BlockTime = block.Timestamp;
        ...
        private void receivedHeightMessage(string address, int port, HeightMsg msg)
        {
            ...
            var node = nodes.FirstOrDefault(n => n.IP == address && n.Port == port);
        ...
        {
            if (RemoteLatestHeight - GlobalParameters.LocalHeight <= 10 && msg.Header.Height - GlobalParameters.LocalHeight <= 10 && !this.tempBlockList.Any(b => b.Header.Height == msg.Header.Height) && !this.blockComponent.CheckBlockExists(msg.Header.Hash))
            {
        ...
        private bool saveBlockToDB(BlockMsg block)
        {
            ...
            else if (block.Header.Height > 0)
            {
                if (this.blockComponent.CheckBlockExists(block.Header.PreviousBlockHash))
                {
                    try
                    {
                        if (!this.blockComponent.SaveBlockIntoDB(block))
                        {
                            return false;
                        }
                    ...
                    {
                        this.blockedBlockHashList.Add(block.Header.Hash);
                ...
                {
                    if (!this.tempBlockList.Any(t => t.Header.Hash.Equals(block.Header.Hash)))
                    {
                        this.tempBlockList.Add(block);
                    }
                }
            ...
            else
            {
                try
                {
                    ...
                    GlobalParameters.LocalHeight = block.Header.Height;
                    GlobalParameters.LatestBlockTime = block.Header.Timestamp;
                    var item = this.tempBlockList.FirstOrDefault(t => t.Header.Height == block.Header.Height);
